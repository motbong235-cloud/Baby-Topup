"""
BABY TOPUP — Python (Flask) backend
...
Serves:
  GET /       -> baby_topup.html (single-file frontend)
...
FRONTEND_PAYLOAD_KEY = os.environ.get(
    "FRONTEND_PAYLOAD_KEY", "AHr9yGDWv76vAyD9VvF3lMBFlUQniOwklQbuKTidIGc="
)
...
# are allowed to open the /baby-cp-7r5t page at all, e.g. "111111,222222".
...
STATIC_DIR = BASE_DIR  # baby_topup.html + baby_admin.html live alongside this file
...
if SITE_PRIVATE:
    ...
    def _require_site_login():
        ...
        if not ok:
            ...
            resp.headers["WWW-Authenticate"] = 'Basic realm="BABY TOPUP (private)"'
            return resp
        return None
...
#
# /baby-cp-7r5t first checks for a valid "baby_admin_device" cookie. If
# missing/invalid it serves a small self-contained gate page (not
# baby_admin.html) asking for a 6-digit code; the code is requested via
...
_DEVICE_COOKIE_NAME = "baby_admin_device"
...
def serve_admin():
    ...
    try:
        return _no_cache_html(STATIC_DIR, "baby_admin.html")
    except FileNotFoundError:
        # baby_admin.html wasn't part of this deploy bundle — every /api/admin-*
        ...
        return json_response(
            {"success": False, "error": "baby_admin.html not found — upload the admin panel HTML alongside server_v16.py"},
            404,
        )
...
def admin_request_otp():
    ...
    for chat_id in ADMIN_OTP_CHAT_IDS:
        _send_telegram_to(chat_id, f"🔐 BABY TOPUP admin login code: `{code}`\nឆាប់ផុតកំណត់ក្នុង 5 នាទី។")
...
def get_site_settings():
    ...
    return json_response({
        "success": True,
        "settings": {
            "SITE_NAME": s.get("site_name") or "BABY TOPUP",
            "FOOTER_NAME": s.get("footer_name") or "BABY TOPUP",
    ...
    try:
        res = requests.post(
            ...
            json={
                ...
                "text": "✅ *TEST NOTIFICATION*\nប្រព័ន្ធ Telegram notification របស់ BABY TOPUP ដំណើរការត្រឹមត្រូវ!",
                "parse_mode": "Markdown",
            },
            timeout=10,
        )
...
if __name__ == "__main__":
    ...
    print(f"BABY TOPUP server running on http://0.0.0.0:{port}")
